"""
Ablation variant: D1 + D2
  - D1 ON  : deeper per-channel feature extractor (2 conv layers)
  - D2 ON  : Dropout(0.1) at chrominance-denoiser bottleneck
...
class Denoiser(Model):
    """D2 ON — Dropout(0.1) at bottleneck."""
    def __init__(self, num_filters, kernel_size=3, activation='relu'):
        ...
        self.dropout      = layers.Dropout(0.1)          # D2 ON
    ...
    def call(self, inputs, training=False):
        x1 = self.conv1(inputs); x2 = self.conv2(x1)
        x3 = self.conv3(x2);     x4 = self.conv4(x3)
        x  = self.dropout(self.bottleneck(x4), training=training)  # D2 ON
        x  = self.up4(x); x = self.up3(x3+x); x = self.up2(x2+x); x = x+x1
...
class LYT(Model):
    ...
    def __init__(self, filters=32, denoiser_cb=None, denoiser_cr=None):
        ...
        self.lum_weight = 0.2                    # D3 OFF: fixed
    ...
    def call(self, inputs, training=False):
        ...
        cb = self.denoiser_cb(cb, training=training) + cb
        cr = self.denoiser_cr(cr, training=training) + cr
        ...
        shortcut = ref
        ref  = self.msef(ref + self.lum_weight * self.lum_conv(lum)) + shortcut
