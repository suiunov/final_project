...
class Denoiser(Model):
    """D2 ON — Dropout(0.1) inserted after bottleneck MHSA."""
    ...
    def call(self, inputs, training=False):
        ...
        x  = self.dropout(x, training=training)  # D2 ON
        x  = self.up4(x)
        x  = self.up3(x3 + x)
        x  = self.up2(x2 + x)
        x  = x + x1
...
class LYT(Model):
    ...
    def __init__(self, filters=32, denoiser_cb=None, denoiser_cr=None):
        super().__init__()
        # D1 OFF: single conv layer per channel
    ...
    def _create_processing_layers(self, filters):
        # D1 OFF: one conv layer (original)
