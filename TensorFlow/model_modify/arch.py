...
class Denoiser(Model):
    def __init__(self, num_filters, kernel_size=3, activation='relu'):
        ...
        self.dropout = layers.Dropout(0.1)  # 改進2: Bottleneck 後加入 Dropout 正則化
    ...
    def call(self, inputs, training=False):
        ...
        x = self.dropout(x, training=training)  # 改進2: Dropout 正則化
        x = self.up4(x)
        x = self.up3(x3 + x)
        x = self.up2(x2 + x)
        x = x + x1
...
class LYT(Model):
    def __init__(self, filters=32, denoiser_cb=None, denoiser_cr=None):
        ...
        self.lum_weight = tf.Variable(0.2, trainable=True, name='lum_injection_weight')  # 改進3: 可學習亮度注入比例
    ...
    def _create_processing_layers(self, filters):
        layerz = [layers.Conv2D(filters, (3, 3), activation='relu', padding='same') for _ in range(2)]  # 改進1: 加深為 2 層
    ...
    def call(self, inputs, training=False):
        ...
        shortcut = ref
        ref = ref + self.lum_weight * self.lum_conv(lum)  # 改進3: 使用可學習比例
        ref = self.msef(ref)
        ref = ref + shortcut
