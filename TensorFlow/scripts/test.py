...
def start_test(dataset, weights, gtmean, args_postprocess=False):
    ...
    for raw_image, corrected_image in tqdm(test_dataset):
            ...
            # Post-processing: CLAHE (Y channel) + Guided Filter
            if args_postprocess:
                img_np = generated_image.numpy()[0]  # (H, W, 3)
                img_np = postprocess(img_np)
                generated_image = tf.expand_dims(tf.constant(img_np, dtype=tf.float32), 0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--postprocess', action='store_true', help='Apply post-processing (CLAHE + Guided Filter).')
    args = parser.parse_args()

    start_test(args.dataset, args.weights, args.gtmean, args.postprocess)
